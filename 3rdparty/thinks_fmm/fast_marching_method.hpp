...
// SPDX-FileCopyrightText: 2017 Tommy Hinks
            ...
            if (has_inside) {
                for (auto j = i + 1; j < connected_components_size; ++j) {
                    ...
                    if (Contains(outer_bbox, inner_bbox)) {
                        // We don't actually care about this error case, which is triggered
                        // by dual contouring of contained components.
               ...
               std::array<T, N> const& grid_spacing)
{
    ...
    // Define and initialise q out of if/else for error reporting convenience
                           ...
                           std::array<T, N> const& grid_spacing)
{
    ...
    for (auto i = std::size_t{0}; i < N; ++i) {
        ...
        // Check neighbors in both directions for this dimension.
